...
{
  "success": false,
  "message": "只能暂停运行中的任务，当前状态为: pending",
  "error_code": "INVALID_STATUS"
}
```
...
{
  "success": false,
  "message": "只能继续已暂停的任务，当前状态为: running",
  "error_code": "INVALID_STATUS"
}
```
...
```http
GET /api/dedup/tasks/1/similar-pairs?page=1&page_size=20&min_similarity=0.85&group_type=1&format=grouped
```
...
- `group_type`: "1" (可选，题型筛选：1=单选, 2=多选, 3=判断, 4=填空, 8=计算分析)
- `format`: "grouped" (可选，返回格式，默认 "grouped")
  - `"grouped"`: 按题目分组格式，每个题目只返回一次，列出所有与它重复的题目（推荐）
  - `"pairs"`: 原始对格式，返回所有重复对（可能包含重复的题目）

**响应数据** (format=grouped，默认格式):
...
**说明**:
- **默认格式 (grouped)**: 按题目分组，每个题目只返回一次，避免重复显示
  - `question_id`: 题目ID
  - `duplicate_count`: 与该题目重复的题目数量
  - `max_similarity`: 最大相似度
  - `min_similarity`: 最小相似度
  - `duplicates`: 与该题目重复的所有题目列表（按相似度降序排列）
- **原始格式 (pairs)**: 返回所有重复对，可能包含重复的题目
- 如果题目A与题目B、C、D都重复，使用 `grouped` 格式时，题目A只会出现一次，并列出所有重复的题目
...
4. **任务状态**: pending（待处理）、running（运行中）、paused（已暂停）、completed（已完成）、error（错误）、cancelled（已取消）

5. **任务状态流转**: 
   - `pending` → `running` → `paused` → `running` → `completed`
   - 任何状态都可以变为 `error` 或 `cancelled`
